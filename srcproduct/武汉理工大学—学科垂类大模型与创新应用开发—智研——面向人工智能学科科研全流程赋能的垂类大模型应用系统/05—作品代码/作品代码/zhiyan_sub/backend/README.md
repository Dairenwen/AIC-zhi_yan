...
迁移会为每个账号补齐一个可编辑的“API 对话模型”基础配置，默认使用 `USER_API_MODEL_BASE_URL` 和 `USER_API_MODEL_NAME`（当前示例配置为 SiliconFlow 的 `deepseek-ai/DeepSeek-V4-Flash`）；已有账号的基础 API 配置也会同步到这两个值，并标记为待验证。迁移完成后，请在个人中心“模型库”中测试连接，再点击“设为默认”。用户仍可继续添加任意数量的个人 API 服务配置。对话模型可以在平台本地模型、账号基础 API 模型和个人 API 模型之间切换。普通用户每个自然月默认拥有 1,000,000 Token 免费额度，可通过 `NORMAL_USER_FREE_TOKEN_QUOTA` 调整；系统管理员不受该额度限制。用量接口为 `GET /api/v1/model-usage`。
...
也可省略上传字段并传入 `link`，支持 arXiv URL 或 `arxiv:` 编号。运行目录、`uv` 命令、缓存、超时和上传大小均可通过 `.env.example` 中的 `PAPER_READING_*` / `PAPER_UPLOAD_*` 配置覆盖。默认关闭 Docling/TableFormer 可选增强（`PAPER_READING_ENABLE_DOCLING=false`），避免基础精读依赖额外的 Hugging Face 下载链；仅在部署机已安装 `tables` extra 及本地模型后再开启。
...
PATENT_DRAFTING_TIMEOUT_SECONDS=900
